...
public class RoleDao {
    ...
    public List<Role> getRoles(Student student){
        final String getRolesSql = "SELECT * from ROLE where IF(?, BTECH = 1, IDD = 1) and MINPASSINGYEAR <= ? and MAXPASSINGYEAR >= ? and MAXACTIVEBACKLOGS >= ? and MAXTOTALBACKLOGS >= ?";
        return jdbcTemplate.query(getRolesSql, new Object[]{student.getProgramme().equals("BTech"),student.getPassingYear(), student.getPassingYear(), student.getActiveBacklogs(), student.getTotalBacklogs()},new BeanPropertyRowMapper<>(Role.class));
    }
